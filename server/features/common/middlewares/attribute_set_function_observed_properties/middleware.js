...
class AttributeSetFunctionObservedProperties extends PlantWorksBaseMiddleware {
	...
	async _getAttributeSetFunctionObservedProperty(ctxt) {
		try {
			...
			attributeSetFunctionObservedPropertyData = this.$jsonApiMapper.map(attributeSetFunctionObservedPropertyData, 'common/attribute-set-function-observed-property', {
				'typeForModel': {
			...
			delete attributeSetFunctionObservedPropertyData.included;
	...
	async _createAttributeSetFunctionObservedProperty(ctxt) {
		try {
			const attributeSetFunctionObservedProperty = ctxt.request.body;
			const jsonDeserializedData = await this.$jsonApiDeserializer.deserializeAsync(attributeSetFunctionObservedProperty);

			jsonDeserializedData['attribute_set_function_observed_property_id'] = jsonDeserializedData['id'];
			delete jsonDeserializedData.id;
			delete jsonDeserializedData.created_at;
			delete jsonDeserializedData.updated_at;

			Object.keys(attributeSetFunctionObservedProperty.data.relationships || {}).forEach((relationshipName) => {
				if(!attributeSetFunctionObservedProperty.data.relationships[relationshipName].data) {
					delete jsonDeserializedData[relationshipName];
					return;
				}

				if(!attributeSetFunctionObservedProperty.data.relationships[relationshipName].data.id) {
					delete jsonDeserializedData[relationshipName];
					return;
				}

				jsonDeserializedData[`${relationshipName}_id`] = attributeSetFunctionObservedProperty.data.relationships[relationshipName].data.id;
			});
			...
			return {
				'data': {
					'type': attributeSetFunctionObservedProperty.data.type,
	...
	async _deleteAttributeSetFunctionObservedProperty(ctxt) {
		try {
			const attributeSetFunctionObservedProperty = await new this.$AttributeSetFunctionObservedPropertyModel({
				'tenant_id': ctxt.state.tenant['tenant_id'],
				'attribute_set_function_observed_property_id': ctxt.params['attributeSetFunctionObservedPropertyId']
			})
			.fetch();

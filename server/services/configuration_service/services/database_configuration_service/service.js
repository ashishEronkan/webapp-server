...
class DatabaseConfigurationService extends PlantWorksBaseService {
	...
	_databaseNotification(data) {
		if(plantworksEnv === 'development' || plantworksEnv === 'test') console.log(`${this.name}::_databaseNotification: ${JSON.stringify(data, null, '\t')}`);
	...
	_databaseQuery(queryData) {
		if((plantworksEnv === 'development' || plantworksEnv === 'test') && this.$config.debug) console.debug(`${this.name}::_databaseQuery: ${JSON.stringify(queryData, undefined, '\t')}`);
	}

	_databaseNotice() {
		if((plantworksEnv === 'development' || plantworksEnv === 'test') && this.$config.debug) console.info(`${this.name}::_databaseNotice: ${JSON.stringify(arguments, undefined, '\t')}`);
	}

	...
	async publish(auditPayload) {
		...
		try {
			...
			if(auditPayload.error) {
				...
				await this.$dependencies.PubsubService.publish('plantworks-audit', 'PLANTWORKS.AUDIT.ERROR', JSON.stringify(auditPayload));
			}
			else {
				...
				await this.$dependencies.PubsubService.publish('plantworks-audit', 'PLANTWORKS.AUDIT.LOG', JSON.stringify(auditPayload));
			}
		}
		catch(err) {

...
class WebsocketService extends PlantWorksBaseService {
			...
			this.$websocketServer.use('plantworks', function() {
				...
				const plantworksPrimusMiddleware = async function(request, response, next) {
					try {
						...
						throw new PlantWorksSrvcError(`WebsocketService::plantworksPrimusMiddleware error`, err);
					}
				};
	...
	_websocketServerLog() {
		...
		if(plantworksEnv === 'development') loggerSrvc.debug(`Websocket Server Log`, JSON.stringify(arguments, undefined, '\t'));
	}
